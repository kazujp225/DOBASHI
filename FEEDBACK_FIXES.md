...
### 修正対象ファイル
- [x] `frontend/src/pages/Collection.tsx` - 複数URL入力対応 ✅ **修正済み**
- [ ] `api/routers/analysis.py` または収集API - 収集後自動分析

### 修正済み内容 (2026-01-13)
- ✅ 複数行テキストエリアで複数URLを入力可能（1行1URL）
- ✅ 順次処理: URLを1つずつ順番に収集（APIの制限を考慮）
- ✅ 進捗表示: 各URLの収集状況をリアルタイム表示
- ✅ 中断機能: 収集中に中断ボタンで停止可能
- ✅ 完了サマリー: 成功/エラー件数を表示
...
### 修正対象ファイル
- [x] `frontend/src/pages/Analysis.tsx` - レイアウト変更、出演虎のみ表示 ✅ **修正済み**

### 修正済み内容 (2026-01-13)
- ✅ 1行1動画のテーブルレイアウトに変更
- ✅ 出演虎（video_tigersに登録された虎）のみ表示
- ✅ 総コメント数を列として表示
- ✅ 出演虎と言及率（rate_total）をバッジ形式で表示
- ✅ 3名以上の場合は展開/折りたたみ機能付き
...
### 修正対象ファイル
- [x] `frontend/src/pages/Analysis.tsx` - エクスポート処理変更 ✅ **修正済み**

### 修正済み内容 (2026-01-13)
- ✅ 縦持ち形式（ロングフォーマット）でCSV出力
- ✅ 各行に: video_id, video_title, total_comments, tiger_id, tiger_name, mention_count, rate_total
- ✅ 出演虎のみをフィルタして出力
- ✅ rate_totalは%表示（小数点2桁）
...
| 中 | 動画分析の出演虎のみ表示 | ユーザビリティ | ✅ 修正済み |
| 中 | CSVエクスポートのローデータ対応 | データ活用 | ✅ 修正済み |
| 中 | 複数URL一括収集 | 効率化 | ✅ 修正済み |
